...
def best_first_search(tube_dict:dict, start_station: str, end_station: str, reverse: bool=False, line_cost:bool=False):
	...
	while agenda:
		...
		if reverse:
			child_nodes = reversed(child_nodes)
		set_child_nodes = []
		line_info = []
		for child in child_nodes:
			station, _, child_line = child
			if station not in explored:
				cost = h(current_state=station, goal_state=end_station)
				total = node_cost + cost
				entry = (station, cost, total)
				if line_cost:
					# We account for line cost if added.
		...
		for index, child in enumerate(set_child_nodes):
			...
			agenda.append({"cost": total, "current": {"station": station, "line": line_info[index]}, "indv_cost": cost,
			               "previous": node})
...
if __name__ == "__main__":
	...
	# Run search
	stations_expanded, node = best_first_search(tube_dict=station_dict, start_station="Harlesden", end_station="Mile End", reverse=False, line_cost=False)
